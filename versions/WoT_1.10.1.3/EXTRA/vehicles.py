...
import struct, itertools, copy, os
from typing import List, Optional, Tuple, Dict, Any, TYPE_CHECKING
import BigWorld
from Math import Vector2, Vector3
import nation_change, nations, items
...
class VehicleDescriptor(object):
    ...
    def installEnhancements(self, enhancements):
    ...
    def maySwapOptionalDevice(self, leftID, rightID):
        if leftID >= self.type.supplySlots.getAmountForType(ITEM_TYPES.optionalDevice):
            ...
            if rightID >= self.type.supplySlots.getAmountForType(ITEM_TYPES.optionalDevice):
                return (False, ('Wrong slotIDx ({})').format(leftID))
            prevDevices = self.optionalDevices
            devices = list(prevDevices)
            self.optionalDevices = devices
    ...
    def getHitTesters(self):
        hitTesters = [
         self.chassis.hitTester, self.hull.hitTester]
        for turretDescr, gunDescr in self.turrets:
...
class VehicleType(object):
    ...
    __slots__ = (
     ...
     'invisibility', 'invisibilityDeltas', 'crewRoles', 'extras', 'extrasDict',
     ...
     'nationChangeGroupId', 'isCollectorVehicle', 'hasTurboshaftEngine', 'hasHydraulicChassis',
     'supplySlots', 'optDevsOverrides')
...
class SupplySlotsStorage(object):
    ...
    def getSlotIDsByType(self, itemType, subItemType=None):
        if subItemType is None:
            return self._slotsByType.get(itemType, {}).get(SupplySlotsStorage.ALL_IDS_KEY, [])
        else:
            return self._slotsByType.get(itemType, {}).get(subItemType, [])
...
class Cache(object):
    __slots__ = ('__vehicles', '__commonConfig', '__chassis', '__engines', '__fuelTanks',
                 ...
                 '__supplySlots', '__supplySlotsStorages')
