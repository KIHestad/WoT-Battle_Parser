...
def _makeExtraNames(tankmanNames):
    ...
    extraSuffix = 'Health'
    edgeCases = {'track': [
               'leftTrack' + extraSuffix, 'rightTrack' + extraSuffix], 
       'radioman': [
...
class VehicleDescriptor(object):
    ...
    __slots__ = ('enhancements', 'turret', 'gun', 'hull', 'engine', 'fuelTank', 'radio',
                 ...
                 'observerPosOnChassis', 'observerPosOnTurret', '_customRoleSlotTypeId',
...
class VehicleType(object):
    ...
    __slots__ = (
     ...
     'supplySlots', 'optDevsOverrides', 'postProgressionTree', 'customRoleSlotOptions', '__weakref__')
    ...
    def __getRoleFromTags(self):
        ...
        if not suitableRoles:
            LOG_WARNING('All vehicles from levels %s must have role, vehicle: %s(CD:%s)' % (
             ROLE_LEVELS, self.name, self.compactDescr))
